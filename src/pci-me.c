...
static int mei_me_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (dma_set_mask(&pdev->dev, DMA_BIT_MASK(64)) ||
		...
		if (err)
			err =/* dma_set_coherent_mask*/dma_set_mask(&pdev->dev,
						    DMA_BIT_MASK(32));
	}
	if (err) {
...
static struct pci_driver mei_me_driver = {
	...
	//.driver.probe_type = PROBE_PREFER_ASYNCHRONOUS, --> not in kernel 2.6.32
};

//module_pci_driver(mei_me_driver);
static int __init mei_me_init(void)
{
	return pci_register_driver(&mei_me_driver);
}

static void __exit mei_me_exit(void)
{
	pci_unregister_driver(&mei_me_driver);
}

module_init(mei_me_init);
module_exit(mei_me_exit);
